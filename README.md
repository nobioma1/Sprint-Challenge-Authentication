...
1. What is the purpose of using _sessions_?
  ...
  ```Sessions are used to authenticate and authorize a user after login. It can be used to track user state.```

2. What does bcrypt do to help us store passwords in a secure manner.

  ```
    Bcrypt hashes a users password upon registration, so the password is not stored as plain in the database and when
    the user is logging in the hash is compared with the login password
  ```

3. What does bcrypt do to slow down attackers?

  ```
    Adding salt to the hashed password that is being stored in the database
  ```
